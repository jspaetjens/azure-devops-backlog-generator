...
| 2.11 | 2026-08-28 | Approved Baseline | Jack Spaetjens | Defined the successful Parent-Child Relationship PATCH response and validation contract. |
...
Implementation status: the full preflight coordinator retrieves project and required metadata evidence, evaluates structural compatibility and submits every exact candidate through validation-only Create in deterministic source order. Deterministic hierarchy traversal consumes the successful `PreflightState` without repeating preflight: roots retain resolution inside the root lifecycle coordinator, while each non-root performs one upstream resolution before the existing non-root lifecycle coordinator receives its eligible parent ID. The traversal reuses the exact validated candidates and existing REST operations. The implemented final Generator-owned preflight-to-traversal entry coordinator composes those existing operations and introduces no REST endpoint, URL, HTTP-method, api-version, authentication, payload, response, timeout, proxy, redirect or retry contract change; the REST Client remains transport-only.
...
Validation-only Create is mandatory before persistent generation for every constructed `WorkItemCandidate` in deterministic source order. Each candidate shall be independently validated using the exact JSON Patch contract intended for its persistent Create, including its generator-produced identity marker and every field the generator would emit for that candidate. A matching work-item type, optional-field presence or JSON Patch shape shall not allow one candidate to serve as validation evidence for another. Validation-only requests shall include `validateOnly=true`, use `application/json-patch+json`, remain non-persisting, follow the common transport contract and shall not retry automatically. All validation-only checks shall succeed before WIQL, Work Item GET, persistent Create, relationship-state GET or Parent-Child Relationship PATCH. The merged Full Preflight Coordinator implements this sequencing.

The Version 1.0 execution order shall be: configuration validation; documentation processing; source-identity generation and run-level collision validation; canonical project retrieval; required work-item type existence validation; type-specific and global field metadata retrieval; structural Scrum compatibility evaluation; candidate JSON Patch construction; mandatory validation-only Create compatibility checks; existing-item resolution; persistent work-item creation; parent-child relationship creation or recovery; and, after normal configured application return, the eligible execution-summary observation (SUMMARY), followed by the independently eligible application COMPLETION lifecycle event.

[Architecture Section 13.4](02-Architecture.md#134-execution-summary-behavioural-contract) owns the
approved execution-summary contract. SUMMARY is success-only, INFO-filtered and best effort in the
existing logfile, without stdout/stderr output. COMPLETION remains the separate existing lifecycle
event and shall not substitute for SUMMARY. Summary implementation and validation remain pending;
existing COMPLETION implementation is not evidence of summary completion. Failure paths retain their
